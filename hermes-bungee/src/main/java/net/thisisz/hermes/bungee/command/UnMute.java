        ...
        if (args.length == 1) {
            Runnable mute = () -> {
                ...
                if (uuids.size() > 1) {
                    ...
                    getPlugin().getMessagingController().setMuted(uuids.keySet().iterator().next(), false);
                    commandSender.sendMessage(new ComponentBuilder(uuids.get(uuids.keySet().iterator().next()) + " has been unmuted!").create());
                } else {
